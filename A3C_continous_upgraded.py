...
class ACNet(object):
        ...
        if scope == GLOBAL_NET_SCOPE:   # get global network
            ...
            with tf.compat.v1.variable_scope(scope):
                ...
                with tf.compat.v1.name_scope('wrap_a_out'):
                    mu, sigma = mu, sigma + 1e-16

                normal_dist = tf.compat.v1.distributions.Normal(mu, sigma)
    ...
    def _build_net(self, scope):
        w_init = tf.compat.v1.initializers.random_normal(0., .1)
        with tf.compat.v1.variable_scope('actor'):
            l_a = tf.compat.v1.layers.dense(self.s, 100, tf.nn.relu6, kernel_initializer=w_init, name='la')
            mu = tf.compat.v1.layers.dense(l_a, N_A, None, kernel_initializer=w_init, name='mu')
